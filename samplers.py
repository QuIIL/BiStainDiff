...
def bibbdm_sampler(
    ...
    """
    Brownian Bridge DDPM-style reverse sampling loop.

    Uses model.bb (BrownianBridgeSchedule) for the step sequence and
    model.inference() for the denoising network forward pass.

    Args:
        model:          SiTBB instance (must have model.bb attribute).
        source_latent:  Encoded source image latent  [B, C, H, W].
                        For 'a2b': this is the HE  latent (a).
                        For 'b2a': this is the IHC latent (b).
        y:              Class / dummy labels  [B].
        direction:      'a2b' (HE→IHC) or 'b2a' (IHC→HE).
        clip_denoised:  Clamp predictions to [-1, 1].
        text_emb:       Text context embedding   [B, D_text].

    Returns:
        Translated latent tensor  [B, C, H, W].
    """
    bb = model.bb
    device = source_latent.device
    steps = bb.steps.to(device)

    # Start from the source endpoint (no additional noise at start)
    x_cur = source_latent.clone()

    with torch.no_grad():
        if direction == "b2a":
            # B→A: iterate t = T-1, T-2, ..., 0
            step_range = range(len(steps))
        elif direction == "a2b":
            # A→B: iterate t = 0, 1, ..., T-1 (reversed)
            step_range = reversed(range(len(steps)))
        else:
            raise ValueError(f"direction must be 'a2b' or 'b2a', got '{direction}'")

        for i in step_range:
            t_idx = steps[i].long()
            t_vec = torch.full((source_latent.shape[0],), t_idx.item(),
                               device=device, dtype=torch.long)
            # Float timestep in [0,1] for the SiTBB TimestepEmbedder
            t_float = t_vec.float() / (bb.num_timesteps - 1)

            # Network prediction
            obj_recon = model.inference(
                x_t=x_cur,
                t=t_float,
                y=y,
                text_emb=text_emb,
